...
public class Student {
    ...
    private final Map<String, CourseEngageement> engageementMap = new HashMap<>();
    ...
    public void addCourse(Course newCourse, LocalDate enrollDate){
        engageementMap.put(
            ...
            new CourseEngageement(newCourse, enrollDate, "Enrollment")
        );
    }
    ...
    public Map<String, CourseEngageement> getEngageementMap() {
        return Map.copyOf(engageementMap);
    }

    public  int getYearsSinceEntrolled(){
        return LocalDate.now().getYear() - yearEnrolled;
    }

    public int getAge(){
        return ageEnrolled + getYearsSinceEntrolled();
    }

    public int getMonthSinceActive(String courseCode){
        CourseEngageement info = engageementMap.get(courseCode);
    ...
    public int getMonthsSinceActive(){
        int inactiveMonths = (LocalDate.now().getYear() - 2012) * 12;
        for (String key : engageementMap.keySet()) {
    ...
    public double getPercentComplete(String courseCode){
        var info = engageementMap.get(courseCode);
    ...
    public void watchLecture(String courseCode, int lectureNumber, int month, int year){
        var activity = engageementMap.get(courseCode);
        if (activity != null){
    ...
    public static Student getRandomStudent(Course... courses){
        ...
        Student student = new Student(
            getRandomVal("AU", "CA", "CN", "GB", "IN", "UA", "US", "KH"),
            random.nextInt(2015, maxYear),
            random.nextInt(18, 90),
            getRandomVal("M", "F", "U", "K"),
            random.nextBoolean(),
            courses
        );
        ...
        for (Course c : courses){
            ...
            if (year == (maxYear - 1)) {
                if (month > LocalDate.now().getDayOfYear()) {
